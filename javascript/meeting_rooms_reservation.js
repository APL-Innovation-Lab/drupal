...
$('#edit-date').change(function(e) {
  ...
  '2024-12-25'
	  ...
	  if (closed_dates.includes(the_date)) {
		...
		function handleInternalCheckboxChange() {
			var internalCheckbox = $('#edit-is-this-meeting-for-internal-use-');
			var groupFieldset = $('#edit-group');
			
			if (internalCheckbox.prop('checked')) {
		  ...
		  // Bind the new function to the change event of the checkbox
		  $('#edit-is-this-meeting-for-internal-use-').change(handleInternalCheckboxChange);
		...
		function check_dates(input_date, room_id) {
          ...
          var endDate = Date.parse('May 19, 2024');
