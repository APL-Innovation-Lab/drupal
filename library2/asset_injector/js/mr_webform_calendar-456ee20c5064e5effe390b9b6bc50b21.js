console.log('MR Webform Calendar');       
          ...
          node.className = 'event status-' + mstatus + ' room-' + mlocation;
            ...
            switch(mlocation) {
                ...
                case '795':
                    ...
                    break;    
                case '5315':
        						var room_name = 'Ruiz Branch #3 - Capacity: 10';
        						break;
                case '796':
...
function calcCollisions(events) {
  collisions = Array(events.length).fill(false).map(() => Array(events.length).fill(false));

  // Track the height of each column
  let columnHeights = Array(events.length).fill(0);
  let columnEndTimes = Array(events.length).fill(0);

  events.forEach((event1, index1) => {
    let currentColumn = 0;

    // Find an empty column for the event
    for (let i = 0; i < columnEndTimes.length; i++) {
      if (columnEndTimes[i] <= event1.start) {
        currentColumn = i;
        break;
      }
    }

    // Update column end time and height
    columnEndTimes[currentColumn] = event1.end;
    columnHeights[currentColumn] += event1.height;

    // Update event's collision data
    events.forEach((event2, index2) => {
      ...
      if (columnHeights[index1] + event1.height > columnHeights[index2] + event2.height) {
        collisions[index1][index2] = true;
        collisions[index2][index1] = true;
      }
    });
...
//updateContainerWidth(maxColumns);
    ...
    if (count > 1) {
      period.forEach((event, id) => {
        if (event) {
          ...
          if (event && leftOffSet[id] === 0) {
            leftOffSet[id] = period.slice(0, id).reduce((a, b) => a + (b ? 1 : 0), 0);
          }
        }
      })
    }
...
function updateContainerWidth(maxColumns) {
    var container = document.getElementById('container'); // assuming the container has id="container"
    container.style.width = (maxColumns * 100) + "px"; // assuming each column is 100px
}
function layOutDay(events) {
  ...
  events.forEach((event, i) => {
    ...
    for (let j = 0; j < i; j++) {
      let other = events[j];
      if (event.start < other.end && event.end > other.start) {  // check for collision
        conflicting.push(other);
      }
    }

    conflicting.sort((a, b) => a.pos - b.pos);
    let position = 0;

    while (conflicting.some(a => a.column === position)) position++;

    event.column = position;
  });
  ...
  events.forEach((event) => {
    ...
    // Here we multiply the column (position in the x axis) by 100px for each column
    let left = event.column * 100;
    if (event.column + 1 > maxColumns) maxColumns = event.column + 1;

    if (!top || top < 0) {
      height = height - Math.abs(top);
      top = 0;
    }
    createEvent(mstatus, internal, mlocation, mtitle, height, top, left, maxColumns);
  });

  // Update container width based on the maximum number of columns used
  updateContainerWidth(maxColumns);
}
